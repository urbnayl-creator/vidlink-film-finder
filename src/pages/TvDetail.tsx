...
const TvDetail = () => {
  ...
  const { data: episodes } = useQuery({ queryKey: ["episodes", tvId, selectedSeason], queryFn: () => getTvSeasonEpisodes(tvId, selectedSeason), enabled: !!tvId && activeTab === "episodes" });
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="flex-1 pt-8 sm:pt-16 md:pt-20">
            <div className="flex flex-wrap items-center gap-2 mb-3">
              <span className="px-2.5 py-1 text-xs font-medium border border-border rounded-md text-foreground">{show.vote_average?.toFixed(1)}</span>
              {show.genres?.map((g) => (<span key={g.id} className="px-2.5 py-1 text-xs border border-border rounded-md text-muted-foreground">{g.name}</span>))}
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3 mb-8">
              <Link to={`/watch/tv/${show.id}?s=1&e=1`} className="btn-glow"><Play className="w-4 h-4" /> Play Now</Link>
            ...
            {activeTab === "overview" && (
              <div className="grid grid-cols-2 gap-4 max-w-lg">
                <div><p className="text-xs text-muted-foreground">First Air Date</p><p className="text-sm text-foreground">{show.first_air_date}</p></div>
                <div><p className="text-xs text-muted-foreground">Status</p><p className="text-sm text-foreground">{show.status}</p></div>
                <div><p className="text-xs text-muted-foreground">Seasons</p><p className="text-sm text-foreground">{show.number_of_seasons}</p></div>
                <div><p className="text-xs text-muted-foreground">Episodes</p><p className="text-sm text-foreground">{show.number_of_episodes}</p></div>
              </div>
            )}
              ...
              <div>
                ...
                <div className="space-y-2">
                  {episodes?.episodes?.map((ep) => (
                    <Link key={ep.id} to={`/watch/tv/${tvId}?s=${ep.season_number}&e=${ep.episode_number}`} className="flex gap-4 p-3 rounded-lg border border-border hover:bg-secondary/50 transition-colors group">
                      <div className="w-32 h-18 rounded-md overflow-hidden bg-secondary shrink-0">
                        {ep.still_path ? <img src={img(ep.still_path, "w300")} alt={ep.name} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-muted-foreground text-xs aspect-video">No Image</div>}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-foreground">E{ep.episode_number}. {ep.name}</p>
